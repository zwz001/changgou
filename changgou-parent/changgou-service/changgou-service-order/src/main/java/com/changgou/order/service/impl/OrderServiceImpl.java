...
public class OrderServiceImpl implements OrderService {
     ...
     * 根据订单ID修改订单状态
     *
     * @param out_trad_no    订单编号
     * @param end_time       交易结束时间
     * @param transaction_id 交易流水号
     */
    @Override
    public void updateStatus(String out_trad_no, String end_time, String transaction_id) {
        //1.根据订单号获取订单的对象
        Order order = orderMapper.selectByPrimaryKey(out_trad_no);
        //2.更新数据
        order.setPayStatus("1");    //已经支付
        order.setUpdateTime(new Date());    //数据库的数据更新时间
    ...
    public void deleteOrder(String out_trade_no) {
        ...
        order.setPayStatus("2");    //2为支付失败
        order.setIsDelete("1");     //1表示已经删除
        orderMapper.updateByPrimaryKeySelective(order); //更新数据库
    }
}
